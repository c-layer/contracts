...
contract("TokenCore", function (accounts) {
  ...
  it("should define audit configuration", async function () {
    ...
    assert.deepEqual(tx.logs[0].args.userKeys.map((x) => x.toString()), ["0", "1"], "userKeys");
  ...
  describe("with a delegate defined", async function () {
    ...
    it("should have an audit configuration", async function () {
      ...
      assert.deepEqual(configuration.userKeys.map((x) => x.toString()), ["0", "1"], "keys");
      ...
      assert.equal(configuration.fields[2], true, "lastEmissionAt");
      assert.equal(configuration.fields[3], true, "lastReceptionAt");
      assert.equal(configuration.fields[4], true, "cumulatedEmission");
      assert.equal(configuration.fields[5], true, "cumulatedReception");
    });
