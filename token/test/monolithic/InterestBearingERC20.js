...
const InterestBearingERC20 = artifacts.require('InterestBearingERC20Mock.sol');
...
contract('InterestBearingERC20', function (accounts) {
  ...
  describe('with an interest rate of +5% per year', function () {
    beforeEach(async function () {
      await token.defineInterest(String(1.05 * ELASTICITY_PRECISION));
      from = await web3.eth.getBlock('latest').then((block) => block.timestamp);
    });
    ...
    describe('after twenty years rebase', function () {
      ...
      it('should rebase ten times', async function () {
        ...
        let elasticity = 1000000000;
        const elasticities = [];
        for (let i = 0; i < 10; i++) {
          elasticity = Math.floor(elasticity * 1.05);
          elasticities.push({
    ...
    describe('with a new interest rate of +3% per year', function () {
      beforeEach(async function () {
        await token.defineInterest(String(1.03 * ELASTICITY_PRECISION));
        from = await web3.eth.getBlock('latest').then((block) => block.timestamp);
      });
  ...
  describe('with a new interest rate of -5% per year', function () {
    beforeEach(async function () {
      await token.defineInterest(String(0.95 * ELASTICITY_PRECISION));
      from = await web3.eth.getBlock('latest').then((block) => block.timestamp);
    });
