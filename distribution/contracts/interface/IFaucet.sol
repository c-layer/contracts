...
abstract contract IFaucet is IVault {
  ...
  event WithdrawLimitUpdate(IERC20 token, uint256 maxBalance, uint256 period);
  ...
  function withdrawLimit(IERC20 _token)
    external virtual view returns (uint256 maxBalance, uint256 period);

  function withdrawStatus(IERC20 _token, address _beneficiary)
    external virtual view returns (uint256 recentlyDistributed, uint64 lastAt);
