...
const T_PLUS_2 = Math.floor(new Date().getTime() / 1000 + 2 * 24 * 3600);
...
contract('ScheduledRouter', function (accounts) {
  ...
  describe('With a route defined', function () {
    ...
    it('should let owner schedule routes', async function () {
      const tx = await router.setRouteSchedule(router.address, T_PLUS_1, T_PLUS_2);
      assert.ok(tx.receipt.status, 'Status');
      assert.equal(tx.logs.length, 1);
      assert.equal(tx.logs[0].event, 'Scheduled', 'event');
      assert.equal(tx.logs[0].args.origin, router.address, 'origin');
      assert.equal(tx.logs[0].args.startAt.toString(), T_PLUS_1.toString(), 'startAt');
      assert.equal(tx.logs[0].args.endAt.toString(), T_PLUS_2.toString(), 'endAt');
    });

    it('should prevent non owner to lock config', async function () {
      await assertRevert(router.setRouteSchedule(router.address,
        T_PLUS_1, T_PLUS_2, { from: accounts[1] }), 'OW01');
    });

    describe('With a route scheduled', function () {
      beforeEach(async function () {
        await router.setRouteSchedule(router.address, T_PLUS_1, T_PLUS_2);
      });

      it('should have no schedule for self', async function () {
        const schedule = await router.schedule(router.address);
        assert.equal(schedule.startAt.toString(), T_PLUS_1.toString(), 'startAt');
        assert.equal(schedule.endAt.toString(), T_PLUS_2.toString(), 'endAt');
      });
    });
